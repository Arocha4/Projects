    ...
    if ip is None: #if non-ip packet	
    	...
    	if switch_id is 1:
    		...
    		if port_on_switch is 1: #from h1
    			msgAction = of.ofp_action_output(port = 2) #send to core switch
    			Msg.actions.append(msgAction)
    			#print 'sent to s4'
    		...
    		if port_on_switch is 1: #from h5
    			...
    			msgAction = of.ofp_action_output(port = 1) #send to server1
    			Msg.actions.append(msgAction)
    		...
    		if port_on_switch is 1: #from h3
    			msgAction = of.ofp_action_output(port = 2) #send to core switch
    			Msg.actions.append(msgAction)
    			#print 'sent to coreswitch'
    		elif port_on_switch is 2: #from core switch
    		...
    		#print 'ip at core swtich'
    		if port_on_switch is 1: 
    			#from untrusted host h4 to server 1 h5, drop
    			if icmp is not None:
    				self.connection.send(Msg)
    				#print 'icmp packet from h4 dropped'
    				return
    			elif ip.dstip == '10.0.4.10': #drop ip packet to h5 from h4
    				self.connection.send(Msg)
    				#print 'ip packet from h4 to h5 dropped'
    				return
    			elif ip.dstip == '10.0.1.10': #forward to h1
    				msgAction = of.ofp_action_output(port = 2)
    				Msg.actions.append(msgAction)
    				#print 'sent to host 1'
    			elif ip.dstip == '10.0.2.20': #forward to h2
    				msgAction = of.ofp_action_output(port = 3)
    				Msg.actions.append(msgAction)
    				#print 'sent to host 2'
    			elif ip.dstip == '10.0.3.30': #forward to h3
    				msgAction = of.ofp_action_output(port = 4)
    				Msg.actions.append(msgAction)
    				#print 'sent to host 3'

    		else:
    			if ip.dstip == '176.16.10.100': #forward to h4
    				...
    				Msg.actions.append(msgAction)
    				#print 'sent o host 3'
    			elif ip.dstip == '10.0.4.10': #forward to h5
